...
Squid.prototype._callGHApplication = function( token )
{
  var github      = this.service('github')
    ...
    , serviceUrl  = '/applications/' + credentials.id + '/tokens/' + token
    , serviceAuth = new Buffer( [ credentials.id, credentials.secret ].join(':') ).toString('base64')
...
// Delete local token and revoke Github auth - WIP
