...
Squid.prototype.setup = function( options )
{
  ...
  // Setup logger
  var transports = []
console.log(this._CONFIG.get())
console.log(options.local)
return
  this._CONFIG.get('logger.transports').forEach( function( config )
  {
    transports.push( new winston.transports[ config.output ]( config ) )
  })
  ...
  {
    this._LOGGER.debug('[Core] Config/Storage/Logger ready')
    this._LOGGER.debug('[Core] version: ' + this._VERSION)
    this._LOGGER.debug('[Core] environement: ' + this._CONFIG.getEnv())
    this._LOGGER.debug('[Core] storage engine: ' + this._STORAGE.getEngine())
  }

  // this._LOGGER.info('[Core] config: ' + JSON.stringify( this ._CONFIG.get() ) )
